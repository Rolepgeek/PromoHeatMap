...
BaseX = sns.clustermap(XL_BaseTableNANless.transpose(), cmap="viridis", figsize=[18, 8], row_cluster=False,
                       # ax=ax, cbar_ax=cbar_ax,
                       xticklabels=True,
                       cbar_kws={'label': 'TPM Values',
                                 # 'fraction': 0.05, 'shrink': 1.0, 'anchor': (0.5, 0.75), 'aspect': 10,
                                 # 'panchor': (0.5, 0.75)
                                 }
                       )
...
plt.savefig("BaseX_SDRLK_111_Express.png",
            bbox_inches='tight'
            )

Log2X = sns.clustermap(XL_Log2TableNANless.transpose(), cmap="viridis", figsize=[18, 8], row_cluster=False,
                       xticklabels=True, cbar_kws={"label": "Log2 Transformed TPM Values"}
                       )
...
plt.savefig("Log2X_SDRLK_111_Express.png",
            bbox_inches='tight'
            )

Log2X = sns.clustermap(XL_Log2TableNANless.transpose(), cmap="viridis", figsize=[18, 8],
                       xticklabels=True, cbar_kws={"label": "Log2 Transformed TPM Values"}
                       )
# Log2X.ax_row_dendrogram.set_visible(False)
Log2DendroBox = Log2X.ax_row_dendrogram.get_position()
...
Log2X.ax_heatmap.set_xlabel("")
Log2X.ax_heatmap.tick_params(axis="x", labelsize=8)
plt.savefig("Log2X_SDRLK_111_ClusTiss.png",
            bbox_inches='tight'
            )